...
COMMANDS: list[Command] = [
    ...
    Command("systemctl", "linux", "sistema", "controla serviços (systemd)", "systemctl <ação> <serviço>",
            ["systemctl status ssh", "sudo systemctl restart nginx"],
            "status é leitura; start/stop/restart mexem no serviço (precisa sudo)."),

    # ---------------- LINUX: rede ----------------
    Command("ping", "linux", "rede", "testa conectividade com um host", "ping <host>",
            ["ping -c 4 8.8.8.8  → 4 pacotes e para"]),
    Command("ip", "linux", "rede", "mostra/config interfaces e rotas", "ip <objeto>",
            ["ip a   → endereços das interfaces", "ip r   → tabela de rotas"]),
    Command("ss", "linux", "rede", "mostra portas/conexões (substitui netstat)", "ss -tulpn",
            ["ss -tulpn  → portas TCP/UDP abertas e quem escuta"]),
    Command("curl", "linux", "rede", "faz requisições HTTP na linha de comando", "curl [opções] <url>",
            ["curl -I https://site  → só os cabeçalhos",
             "curl -s url | less  → silencioso, pra ler a resposta"]),
    Command("wget", "linux", "rede", "baixa arquivos da web", "wget <url>",
            ["wget -O nome.zip url  → salva com esse nome"]),
    Command("dig", "linux", "rede", "consulta DNS (resolução de nomes)", "dig <domínio>",
            ["dig +short exemplo.com  → só o IP",
             "dig erp.company.local  → útil pra diagnosticar NXDOMAIN"],
